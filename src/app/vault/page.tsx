    ...
    <div className="min-h-screen px-6 md:px-16 lg:px-24 py-20">
      <div className="max-w-[1400px] mx-auto">
        ...
        >
          ...
          <h1 className="text-4xl md:text-6xl font-light tracking-tight text-fg mb-4">
            The Vault
          </h1>
          <p className="text-base text-fg-muted max-w-lg">
            Discover premium AI skills. Each one verified, onchain,
            and ready to plug into your agent via MCP.
          </p>
        </motion.div>
        ...
        <motion.div
          ...
          className="flex flex-wrap items-center gap-3 mb-12 border-b border-border pb-6"
        >
          {["All", "CLAUDE", "GPT4", "LLAMA", "GEMINI"].map((tag) => (
            <button
              key={tag}
              className="px-4 py-1.5 text-[10px] font-mono tracking-wider border border-border text-fg-muted hover:text-fg hover:border-border-hover transition-all duration-200"
            >
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-border">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="bg-bg h-72 animate-shimmer"
              />
            ))}
          </div>
        ) : skills.length > 0 ? (
          ...
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-center py-32 border border-border"
          >
            <p className="font-mono text-[10px] tracking-widest text-fg-dim mb-6">
              [empty_state]
            </p>
            <h3 className="text-xl font-light text-fg-muted mb-2">
              The Vault is empty
            </h3>
